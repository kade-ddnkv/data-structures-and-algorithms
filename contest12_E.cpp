...
// Взял B-дерево из предыдущих контестов (+ метод upperBound).
...
class BTree {
    ...
    void getUpperBoundRecursive(Node *curr, int64_t key, int64_t *upper_bound) {
        if (curr->is_leaf_) {
            for (int64_t i = 0; i < curr->size_; ++i) {
                if (curr->keys_[i] >= key) {
                    *upper_bound = curr->keys_[i];
                    return;
                }
            }
            return;
        ...
        for (int64_t i = 0; i < curr->size_; ++i) {
            if (key < curr->keys_[i]) {
                *upper_bound = curr->keys_[i];
                getUpperBoundRecursive(curr->child_ptrs_[i], key, upper_bound);
                return;
            } else if (key == curr->keys_[i]) {
                *upper_bound = curr->keys_[i];
                return;
            }
        }
        ...
        // Если не было запущено ранее, поиск в самом правом потомке.
        getUpperBoundRecursive(curr->child_ptrs_[curr->childrenCount() - 1], key, upper_bound);
    }
...
public:
    ...
    int64_t upperBound(int64_t key) {
        ...
        getUpperBoundRecursive(root_, key, &upper_bound);
        return upper_bound;
    }
...
void handleConsoleCommands(BTree set) {
    int number_of_commands;
    std::cin >> number_of_commands;
    char prev_command;
    int prev_command_result;
    char command = 'c';
    int entered_number;

    for (int tmp = 0; tmp < number_of_commands; ++tmp) {
        prev_command = command;
        std::cin >> command;
        std::cin >> entered_number;
        if (command == '+') {
            if (prev_command == '?') {
                set.insert((entered_number + prev_command_result) % 1000000000);
            } else {
                set.insert(entered_number);
            }
        } else if (command == '?') {
            prev_command_result = set.upperBound(entered_number);
            std::cout << prev_command_result << "\n";
        }
    }
}
