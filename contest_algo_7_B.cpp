...
std::vector<int> getThinEdgesArray(std::string str) {
    std::vector<int> br = getEdgesArray(str);
    std::vector<int> brs(str.size());
    brs[0] = 0;
    int str_size = str.size();
    str.push_back('[');
...
std::vector<int> knuthMorrisPrattAlgorithm(std::string substr, std::string text) {
    std::vector<int> brs = getThinEdgesArray(substr);
